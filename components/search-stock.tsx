  ...
  const filteredTickers =
    query === ""
      ...
      : watchlist.filter((item) =>
        item.symbol.toLowerCase().includes(query.toLowerCase().replace(/\s+/g, ""))
      );
      ...
      <Combobox value={ticker} onChange={setTicker}>
        <div className="relative">
          ...
          <Transition
            ...
            afterLeave={() => setQuery("")}
          >
            <Combobox.Options className="absolute mt-2 ml-8 max-h-60 overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm w-[220px]">
              {filteredTickers.map((item) => (
                <Combobox.Option
                  key={item.symbol}
                  className={({ active }) =>
                    `relative cursor-default select-none py-2 pl-4 pr-4 ${active ? "bg-teal-600 text-black" : "text-gray-900"
                    }`
                  }
                  value={item.symbol}
                >
